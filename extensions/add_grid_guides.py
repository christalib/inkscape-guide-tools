...
def drawGuide(position, orientation, parent):
    ...
    inkex.etree.SubElement(parent, '{http://sodipodi.sourceforge.net/DTD/sodipodi-0.dtd}guide', {
                           'position': positionString, 'orientation': orientationString})
...
class Grid_Guides(inkex.Effect):
    ...
    def effect(self):
        ...
        if (tab == "\"columns\""):
            ...
            drawDoubleGuides(cols, col_width, col_gut, hor_start,
                             has_outer_col_gutter, "vertical", namedview)

            # Give total width in original units
            if (show_total_width == True):
                printDebug("Total width of grid will be: " +
                           str(total_col_width/col_factor) + " " + self.options.column_unit)
            ...
            # Create row guides
            drawDoubleGuides(rows, row_height, row_gut, vert_start,
                             has_outer_row_gutter, "horizontal", namedview)

            # Give total height in original units
            if (show_total_height == True):
                printDebug("Total height of grid will be: " +
                           str(total_row_height/row_factor) + " " + self.options.row_unit)
